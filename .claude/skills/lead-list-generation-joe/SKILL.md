...
description: Baut eine anrufbare Käuferliste für den Verkauf eines Unternehmens. Findet passende Käufer-Firmen samt LinkedIn-Seite, scrapt über Apify die Entscheider dieser Firmen, prüft jede Person auf Passung und reichert E-Mail und Mobilnummer über BetterContact und Lusha an. Ergebnis ist eine Excel-Datei, die man von oben nach unten abtelefonieren kann. Auslösen bei Käuferliste bauen, Käufer für mein Unternehmen finden, Exit-Liste, Anrufliste für den Verkauf, wer könnte meine Firma kaufen, lead list generation, Liste anreichern mit Telefonnummern.
---

# Käuferliste bauen

Du verkaufst dein Unternehmen und willst Käufer anrufen. Dieser Skill baut dafür die
Liste: von der Frage "wer käme überhaupt in Frage" bis zur Excel-Datei mit Name,
...
```
  1  Käuferprofil klären          Gespräch           ->  keine Datei, aber die Grundlage
  2  Firmen + LinkedIn-Seiten     Recherche          ->  01_firmen.csv
  3  Personen von LinkedIn        Apify              ->  02_personen.csv        (kostet)
  4  Passung prüfen               Claude liest       ->  03_personen_gefiltert.csv
  ...
  7  Alles zusammenführen         Excel              ->  06_kaeuferliste.xlsx
```
...
Realistische Erwartung: von den Entscheidern, die überhaupt durchlaufen, bekommen
etwa **30 bis 45 Prozent eine Mobilnummer**, E-Mail deutlich mehr. Wer 200 Leute
anrufen will, braucht also ungefähr 500 geprüfte Personen im Trichter. Rechne von
hinten, nicht von vorne.

## Schritt 1 - Käuferprofil klären

**Nie überspringen.** Ohne dieses Gespräch recherchiert man 60 Firmen, von denen 50
nie gekauft hätten.
...
Das Ergebnis ist ein kurzer Steckbrief, den du während des ganzen Laufs sichtbar
hältst - er ist der Maßstab für jede spätere Ja/Nein-Entscheidung.

## Schritt 2 - Firmen finden und die LinkedIn-Seite dazu

Recherchiere die Käufer-Firmen über die Websuche. Die Käufertypen und die
...
| `linkedin_company_url` | die vollständige LinkedIn-Firmenseite |
| `typ` | welcher Käufertyp (strategisch, Beteiligung, Aggregator, ...) |
| `begruendung` | ein Satz: warum genau die Firma kaufen würde |

### Die teuerste Falle des ganzen Laufs

**Eine geratene LinkedIn-Firmen-URL macht den kompletten Rest wertlos.**
`linkedin.com/company/mustermann` sieht richtig aus und gehört einer anderen Firma
mit ähnlichem Namen. Der Scraper holt dann brav deren Mitarbeiter, die Anreicherung
findet brav deren Nummern, und niemand merkt es - bis jemand am Telefon sagt, dass er
noch nie etwas von dir gehört hat.

Deshalb gilt: **jede LinkedIn-URL wird geöffnet und gegen die Domain geprüft.**
Stimmt die Website auf der LinkedIn-Seite nicht mit `domain` überein, lass das Feld
leer und schreib den Zweifel in `begruendung`. Eine leere Zelle kostet eine Firma.
Eine falsche kostet die Glaubwürdigkeit beim ersten Anruf.

Sag am Ende von Schritt 2 laut, wie viele Firmen gefunden wurden und **bei wie vielen
die LinkedIn-Seite fehlt**. Beide Zahlen gehören zum Ergebnis.
...
python3 .claude/skills/lead-list-generation-joe/scripts/apify_employees.py \
    ...
    --titel "CEO,Geschäftsführer,Managing Director,M&A,Corporate Development,Head of Acquisitions,Investment Manager,Partner"
```
...
als beim ersten Mal 40 Assistenzkräfte je Konzern zu bezahlen.

## Schritt 4 - Passung prüfen
...
ist der Schritt, der die späteren Anreicherungskosten halbiert.

Je Person zwei Werte:

- `passung_score` von 1 bis 10
- `passung_begruendung`: ein Satz, warum. Kein "passt gut", sondern **die Rolle im
  Kaufprozess**: "entscheidet über Zukäufe", "führt die Prüfung durch",
  "trägt das Budget", "kennt den Markt, entscheidet aber nicht".
...
- Doppelte Einträge derselben Person
...
mit einem Firmennamen.** Die Domain steht schon in `01_firmen.csv` - übertrag sie.

Sag danach, wie viele von wie vielen übrig geblieben sind. Wenn fast alle
überleben, war der Filter zu weich.

## Schritt 5 und 6 - E-Mail und Mobilnummer

Zwei Anbieter, nacheinander, **nicht** wahlweise. Sie finden verschiedene Leute; erst
zusammen ergeben sie eine brauchbare Abdeckung. Wer nur einen nimmt, lässt rund ein
...
Lusha fragt standardmäßig nur die Personen an, bei denen BetterContact nicht beides
gefunden hat. Das spart Guthaben. `--alle` fragt alle an, wenn du zwei unabhängige
Nummern je Person willst.

Die Fallen beider Anbieter stehen in `references/anreicherung.md`. Eine davon ist
wichtig genug für hier:
...
> löschen**, auch nicht beim Aufräumen.

> **Ein `2xx` heißt nicht, dass Ergebnisse da sind.** Während die Anreicherung
> läuft, antwortet BetterContact mit `202` und einem leeren Datenfeld. Wer nur auf
> "hat geklappt" prüft, liest das leere Feld und meldet "nichts gefunden". Geprüft
...
Ergebnis: `output/06_kaeuferliste.xlsx`, drei Blätter, Anrufbare oben.
...
falschen Nummer weiss, wer sie geliefert hat, und beim nächsten Mal anders gewichtet.

## Pflicht-Prüfung, bevor du die Datei übergibst

Kein "fertig", bevor diese fünf Punkte gemessen sind. Nicht geschätzt, gemessen:
...
2. **Stichprobe von fünf Zeilen**: passt die Firma in der Zeile zur LinkedIn-URL
...
5. **Stimmt die Zahl der Firmen** im Blatt Firmen mit dem überein, was du in
...
- **Er schreibt keine Mails und ruft niemanden an.** Er baut die Liste. Das Gespräch
  führst du.
- **Er bewertet nicht, was dein Unternehmen wert ist.** Andere Aufgabe, andere Leute.
- **Er findet keine Käufer ohne Internet-Spur.** Eine Holding ohne Website und ohne
  LinkedIn taucht nicht auf. Das ist eine echte Lücke, keine Formsache - für solche
  Käufer bleiben Berater und Branchenkontakte.
...
Du verarbeitest Kontaktdaten echter Menschen. In der EU heißt das: es muss ein Grund
dafür geben (berechtigtes Interesse an einer geschäftlichen Anbahnung ist einer),
die Leute dürfen widersprechen, und du musst auf Nachfrage sagen können, woher die
...
Anruf bei einer Firma zur Anbahnung eines konkreten Geschäfts steht deutlich besser
da als eine Massenmail. Genau darum ist die Mobilnummer hier die Zielgröße.
